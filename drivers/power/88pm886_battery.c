...
#define PM886_GPADC_RTC_SPARE0	(0xc0)
...
int sec_hal_fg_reset(struct pm886_battery_info *info,
...
static int pm886_battery_get_battery_status(struct pm886_battery_info *info)
{
	union power_supply_propval value;
	int status;
 ...
 */
static bool system_may_reboot(struct pm886_battery_info *info)
{
	if (!info || !info->chip)
		return true;
			       ...
			       struct ccnt *ccnt_val)
{
	...
	/* Prevent the ocv value to be read when charging */
...
static void pm886_battery_correct_soc(struct pm886_battery_info *info,
				      struct ccnt *ccnt_val)
{
	static int chg_status, old_soc, bat_status;
	static int power_off_cnt, extreme_power_off_cnt;
	...
	bat_status = pm886_battery_get_battery_status(info);
		...
		if (is_extreme_low) {
			...
			if (info->bat_params.volt <= info->power_off_th) {
				if (power_off_cnt > 4) {
					...
					dev_info(info->dev, "hit power off cnt = %d\n", power_off_cnt);
					power_off_cnt++;
				}
			}
			if (info->bat_params.volt < info->safe_power_off_th) {
...
static void pm886_bat_update_status(struct pm886_battery_info *info)
{
	...
	dev_dbg(info->dev, "%s: volt: %d, ibat: %d, soc: %d%%, charger status: %d, temp: %d, batt health: %d, last_cc: %d, slp_cnt: %d\n",
	__func__, info->bat_params.volt, info->bat_params.ibat, info->bat_params.soc,
	info->bat_params.status, info->bat_params.temp, info->bat_params.health, ccnt_data.last_cc, pm886_battery_get_slp_cnt(info));
}
				 ...
				 int *initial_soc, int *initial_cycles)
{
	...
	int remove_th, chg_status, rtc_spare_0;
	...
	regmap_read(info->chip->gpadc_regmap, PM886_GPADC_RTC_SPARE0, &rtc_spare_0);
	pr_info("%s: Initial Spare 0: 0x%x\n", __func__, rtc_spare_0);
	/* Check for JIG connection */
	if (sec_bat_check_jig_status()) {
		pr_info("%s: Bootup by JIG !!!!\n", __func__);
		regmap_write(info->chip->gpadc_regmap, PM886_GPADC_RTC_SPARE0, rtc_spare_0 | 0x01);
		regmap_read(info->chip->gpadc_regmap, PM886_GPADC_RTC_SPARE0, &rtc_spare_0);
		*initial_soc = soc_from_vbat_active;
		*initial_cycles = 0;
		pr_info("%s: RTC Spare 0: 0x%x\n", __func__, rtc_spare_0);
		goto end;
	} else if (rtc_spare_0 & 0x01) {
	...
	if ((info->chip->powerup & 0x2) && (info->chip->powerdown2 & 0x1)) {
			*initial_soc = soc_from_saved;

		dev_info(info->dev, "HYB_DONE : soc_from_vbat_slp=%d, soc_from_vbat_active=%d, soc_from_saved=%d\n",
				soc_from_vbat_slp, soc_from_vbat_active, soc_from_saved);
	...
	if (system_may_reboot(info)) {
		...
		if (saved_is_valid < 0) {
			...
			*initial_soc = soc_from_vbat_active;
		} else if ((soc_from_vbat_active - soc_from_vbat_slp) > 5) {
			*initial_soc = soc_from_vbat_active;
		} else {
			if (soc_from_saved < 5)
				remove_th = 8;
			...
			if (!!(abs(soc_from_vbat_slp - soc_from_saved) > remove_th))
				*initial_soc = soc_from_vbat_slp;
			else
				*initial_soc = soc_from_saved;

			dev_info(info->dev,
				"%s: soc_from_vbat_slp = %d, soc_from_saved = %d, remove = %d\n",
				__func__, soc_from_vbat_slp, soc_from_saved, remove_th);
	...
	*initial_cycles = cycles_from_saved;
	...
	if (soc_in_good_range) {
		if (abs(soc_from_vbat_slp - soc_from_saved) < 15) {
			*initial_soc = soc_from_saved;
		} else {
			*initial_soc = soc_from_vbat_slp;
		}
			       ...
			       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP_AMBIENT:
		iio_read_channel_scale(channel, &ohm, NULL);
		val->intval = ohm;
		break;
	default:
		return -ENODEV;
...
static int pm886_batt_set_prop(struct power_supply *psy,
			       enum power_supply_property psp,
...
static int pm886_battery_probe(struct platform_device *pdev)
{
	...
	if (info->use_ntc) {
		dev_dbg(chip->dev, "%s : gpadc_det_no : %d\n", __func__, info->gpadc_det_no);
	}
	...
	info->battery.set_property = pm886_batt_set_prop;
