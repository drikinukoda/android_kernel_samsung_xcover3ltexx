...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[pxa_core_clk_chg] state: %u freq: %u -> %u@", CLK_CHG_ENTRY, cop->pclk, top->pclk);
#endif
	/* 0) pre FC */
	pre_fc(apmu_base);
...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[pxa_core_clk_chg] state: %u freq: %u -> %u@", CLK_CHG_EXIT, cop->pclk, top->pclk);
#endif
...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[pxa_ddr_clk_chg] state: %u freq: %u -> %u@", CLK_CHG_ENTRY, old->dclk, new->dclk);
#endif
	local_irq_save(flags);
...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[pxa_ddr_clk_chg] state: %u freq: %u -> %u@", CLK_CHG_EXIT, old->dclk, new->dclk);
#endif
...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[pxa_axi_clk_chg] state: %u freq: %u -> %u@", CLK_CHG_ENTRY, cop->aclk, top->aclk);
#endif
...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[pxa_axi_clk_chg] state: %u freq: %u -> %u@", CLK_CHG_EXIT, cop->aclk, top->aclk);
#endif
}
