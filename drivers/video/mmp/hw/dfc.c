...
static void dynamic_change_dsipll(unsigned int rate, int type)
{
	...
	default:
		if (PXA1928 != plat) {
			...
			/*
			 * FIXME: if DSIPLL is already used by lcd;
			 * then when we want to set lcd to another rate
			 * on PLL4, need to disable PLL4 first, then set
			 * rate.
			 */
			atomic_set(&enable_count, 0);
			while (__clk_is_enabled(dsipll)) {
				clk_disable_unprepare(dsipll);
			...
			while (atomic_dec_and_test(&enable_count))
				clk_prepare_enable(dsipll);
		}
		break;
	}
...
int dfc_request(struct notifier_block *b, unsigned long val, void *v)
{
	...
	if (val == DIP_START){
		rate = *((u32 *)v);
		pr_info("%s DIP_START (rate:%d)", __func__, rate);
	} else {
		rate = original_rate / MHZ_TO_HZ;
		pr_info("%s DIP_END (rate:%d)", __func__, rate);
	}
