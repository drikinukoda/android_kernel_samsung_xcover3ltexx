		...
		const char *propname, struct gen_cmds_info *cmd)
{
	...
	memcpy(data, val, sz);
	ret = init_dcs_cmds(data, sz, propname, cmd);
...
int init_dcs_cmds(char *data, int sz, const char *propname,
		struct gen_cmds_info *cmd)
{
	struct gen_cmd_hdr *gchdr;
	struct gen_cmd_desc *desc;
	int nr_desc = 0, nr, i = 0;
	...
	return 0;
err_alloc_data:
	pr_info("%s: err_alloc_data\n", __func__);
	for (nr = 0; nr < nr_desc; nr++) {
		kfree(desc[nr].data);
		desc[nr].data = NULL;
	}
err_alloc_cmds:
	pr_info("%s: err_alloc_cmds\n", __func__);
	kfree(cmd->name);
	cmd->name = NULL;
	kfree(cmd->desc);
	cmd->desc = NULL;
