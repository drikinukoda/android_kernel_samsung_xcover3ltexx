...
static int tsp_ta_gpio;
static bool ta_tsp_connected;
bool on_off_flag;
...
#if !(CYTTSP5_REPORT_DATA_FROM_TABLE)
...
static void cyttsp5_ta_cb(int ta_status)
{

	ta_status = (ta_status != POWER_SUPPLY_TYPE_BATTERY);
	if (ta_status)
		ta_tsp_connected = true;
	else
		ta_tsp_connected = false;
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cd->bus_ops = ops;
	tsp_ta_gpio = cd->pdata->core_pdata->ta_gpio;
	...
	cd->cpdata->init(cd->cpdata, 1, dev);
...
#if USE_TSP_TA_CALLBACKS
	rc = gpio_request(tsp_ta_gpio, "tsp_ta_gpio");
	if (rc < 0)
		pr_err("%s, gpio %d request failed (%d)\n",
			__func__, tsp_ta_gpio, rc);
	else
		tsp_charger_status_cb = cyttsp5_ta_cb;
