...
static int bt432_power(struct bt432_ts_info *info, int on)
{
	...
	if (touch_regulator == NULL) {
		touch_regulator = regulator_get(&info->client->dev, "tsp_vdd_3v1");
	...
	if (on == 0) {
		...
		if (!is_power_on) {
			is_power_on = 1;
			regulator_set_voltage(touch_regulator, 3100000, 3100000);
			 ...
			 struct bt432_ts_platform_data *pdata)
{
	...
	pdata->gpio_int = of_get_named_gpio(np,"irq-gpio", 0);
	if (pdata->gpio_int < 0) {
