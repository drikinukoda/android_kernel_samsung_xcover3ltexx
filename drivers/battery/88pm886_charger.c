...
/* global pointer to info struct*/
struct pm886_charger_info *g_info;
...
static int pm886_get_charging_health(struct pm886_charger_info *info)
{
	...
	if ((stat2 & PM886_VBUS_SW_OV) && (stat2 & PM886_USB_LIM_OK)){
		dev_info(info->dev, "%s : Health in full status \n", __func__);
...
static void pm886_otg_control(struct pm886_charger_info *info, int enable)
{
	...
	dev_info(info->dev, "%s: PM886_CHG_CONFIG1(0x%x) = 0x%x\n",
...
static irqreturn_t  pm886_chg_det_handler(int irq, void *irq_data)
{
	...
	schedule_delayed_work(&info->isr_work, HZ * 0.2);
	return IRQ_HANDLED;
}
*/
...
int pm886_disable_chg_flash(void)
{
	int chg_online = g_info->is_charging;

	dev_info(g_info->dev, "%s, chg_online = %d \n", __func__, chg_online);

	if (chg_online)
		pm886_stop_charging(g_info);

	return chg_online;
}
EXPORT_SYMBOL(pm886_disable_chg_flash);

int pm886_enable_chg_flash(void)
{
	int chg_online = g_info->is_charging;
	...
	if (chg_online)
		pm886_start_charging(g_info);

	return 0;
}
EXPORT_SYMBOL(pm886_enable_chg_flash);
...
static int pm886_charger_probe(struct platform_device *pdev)
{
	...
	g_info = info = devm_kzalloc(&pdev->dev, sizeof(struct pm886_charger_info),
			GFP_KERNEL);
