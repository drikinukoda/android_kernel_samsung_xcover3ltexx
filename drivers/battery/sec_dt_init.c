...
extern void reset_charger_status(int status);
...
#if defined(CONFIG_SENSORS_AK09911C) && defined(CONFIG_MACH_XCOVER3LTE)
extern void ak09911c_charger_status_cb(int status);
#endif
...
#if defined(CONFIG_SENSORS_AK09911C) && defined(CONFIG_MACH_XCOVER3LTE)
	/* for magnetic sensor compensation at TA/USB status - st480.c */
	ak09911c_charger_status_cb(current_cable_type);
#endif

	/* for PMIC watchdog reset at TA/USB status */
	reset_charger_status(current_cable_type);
			 ...
			 sec_battery_platform_data_t *pdata)
{
	...
	ret = of_property_read_u32(np, "vf_adc_channel",
			&pdata->vf_adc);
	if (ret)
		return ret;

	ret = of_property_read_u32(np, "online_gp_bias_curr",
			&pdata->online_gp_bias_curr);
	if (ret)
		return ret;

	ret = of_property_read_u32(np, "hi_volt_online",
			&pdata->hi_volt_online);
	if (ret)
		return ret;

	ret = of_property_read_u32(np, "lo_volt_online",
