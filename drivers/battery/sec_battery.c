...
static void sec_bat_swelling_check(struct sec_battery_info *battery, int temperature)
{
	...
	pr_info("%s: status(%d), swell_mode(%d), swell_block(%d), chg_float_voltage(%d), cv(0x%x), temp(%d), limit(%d)\n",
		__func__, battery->status, battery->swelling_mode, battery->swelling_block, battery->pdata->chg_float_voltage, val.intval, temperature, BATT_SWELLING_HIGH_TEMP_BLOCK);
	...
	if (!battery->swelling_mode) {
		...
		if (battery->swelling_block_passed < BATT_SWELLING_BLOCK_TIME) {
			pr_info("%s: swelling_timer doesn't reach 10 minutes, stop charging\n", __func__);
		} else {
