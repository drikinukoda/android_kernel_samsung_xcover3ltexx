...
#else
static int prox_led_onoff(struct cm36672p_data *data, bool onoff)
{
	int ret;
	struct regulator *leda;

	leda = devm_regulator_get(&data->i2c_client->dev, "cm36672p,leden");
	if (IS_ERR(leda)) {
...
irqreturn_t proximity_irq_thread_fn(int irq, void *user_data)
{
	...
	if (enabled) {
		...
		memcpy(buf + 1, &data->timestamp, sizeof(data->timestamp));
