...
struct k2hh_p {
	...
	int irq_state;
	int acc_int1;
#if defined(CONFIG_SENSORS_K2HH_SW_I2C)
	int sda_gpio;
	int scl_gpio;
#endif
#if defined(CONFIG_SENSORS_ACC_EN_GPIO)
	int enable_gpio;
#endif
	int time_count;
...
#if defined(CONFIG_SENSORS_ACC_EN_GPIO)
static int k2hh_setup_sensor_en(struct k2hh_p *data)
{
	int ret = 0;

	if (gpio_is_valid(data->enable_gpio)) {
		ret = gpio_request(data->enable_gpio, "ACC_EN");
		if (ret < 0) {
			pr_err("[SENSOR] %s - gpio %d request failed (%d)\n",
				__func__, data->enable_gpio, ret);
		}

		ret = gpio_direction_output(data->enable_gpio, 1);
		if (ret)
			pr_err("[SENSOR] %s: unable to set_direction for acc_en [%d]\n",
...
#if defined(CONFIG_SENSORS_ACC_EN_GPIO)
	data->enable_gpio = of_get_named_gpio_flags(dNode, "stm,enable_gpio",
		0, &flags);
	if (data->enable_gpio < 0)
		pr_err("%s, get acc_enable_gpio error\n", __func__);
#endif
...
static int k2hh_regulator_onoff(struct k2hh_p *data, bool onoff)
{
	...
	if (!data->vdd) {
		data->vdd = devm_regulator_get(&data->client->dev,
			"stm,reg_vdd");
		if (IS_ERR(data->vdd)) {
...
#if defined(CONFIG_SENSORS_ACC_EN_GPIO)
	ret = k2hh_setup_sensor_en(data);
	if (ret)
		pr_err("[SENSOR]: %s - could not enable sensor_en\n", __func__);
#endif
