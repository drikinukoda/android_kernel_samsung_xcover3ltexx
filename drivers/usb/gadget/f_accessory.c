...
#ifdef CONFIG_COMPAT  //for 64bit kernel & 32bit platform
static long acc_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret;
	ret = acc_ioctl(file, cmd, arg);
	return ret;
}
#endif
