...
static int wait_for_usb_phy_ready(struct mv_usb2_phy *mv_phy)
{
	struct platform_device *pdev = mv_phy->pdev;
	unsigned int loops = 0;
	void __iomem *base = mv_phy->base;
	...
	*/
	...
	if (loops >= 100) {
		dev_warn(&pdev->dev, "USB PHY SQ Calibrate not done after 100mS.");
		return -1;
	}
...
static int _mv_usb2_phy_28nm_tune(struct mv_usb2_phy *mv_phy, bool state)
{
...
#if defined(CONFIG_MACH_DEGASVELTE)
		writel(readl(base + PHY_28NM_RX_REG1) |
			((0x1 << PHY_28NM_RX_EXT_SQ_AMP_CAL_EN_SHIFT) | (0x0 << PHY_28NM_RX_EXT_SQ_AMP_CAL_SHIFT)),
			base + PHY_28NM_RX_REG1);
#else
		writel(readl(base + PHY_28NM_RX_REG1) |
	...
	tmp = wait_for_usb_phy_ready(mv_phy);
	if(tmp != 0) {
		pr_err_once("usb: %s: USB Phy READY not set\n", __func__);
	}
