...
#define PM886_BK_OSC_CTRL3		(0x52)
...
#ifdef CONFIG_DEBUG_FS
/* for 88pm886 reg dump debug module */
int power_regmap_bulk_read(unsigned char *buf, unsigned int offs, size_t val_count)
{
	struct regmap *map;
	map = pm886_get_chip()->power_regmap;
	return regmap_bulk_read(map, offs, buf, val_count);
}
EXPORT_SYMBOL(power_regmap_bulk_read);
#endif
...
static const struct reg_default pm886_base_patch[] = {
	...
	{PM886_BK_OSC_CTRL3, 0xc0}, /* set the duty cycle of charger DC/DC to max */
};
...
#endif
#ifdef CONFIG_PMICWD_POWERONOFF
	if (ta_connected) {
				   ...
				   unsigned long code, void *cmd)
{
	...
	if (cmd) {
		if (!strcmp(cmd, "recovery")) {
			...
			if (!strncmp(cmd, "", 0)) {
				/* Disable the LDO7 */
				regmap_update_bits(chip->power_regmap, 0x09, 1 << 6, 0 << 6);
			}
#endif
