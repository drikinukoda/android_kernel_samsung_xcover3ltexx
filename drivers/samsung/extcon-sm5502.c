...
#define DEV_JIG_ALL		(DEV_JIG_UART_OFF | DEV_JIG_UART_ON \
 					| DEV_JIG_USB_OFF | DEV_JIG_USB_ON)
#define DEV_JIG_WAKEUP		(DEV_JIG_UART_OFF | DEV_JIG_UART_ON \
					| DEV_JIG_USB_ON)
...
#define LKP_M			(1 << 3)
#define KP_M			(1 << 2)
#define INTMASK1_INIT		(OVP_OCP_DIS_M | OCP_EVENT_M | OVP_EVENT_M \
					| LKR_M | LKP_M | KP_M)
	...
	if (intr1 & ATTACHED
		...
		if (val2 & DEV_JIG_UART_OFF) {
			if (vbusin & VBUSIN_VALID) {
				dev_classifi = SM5502_CABLE_TYPE2_JIG_UART_OFF_VB;
				dev_info(&client->dev,
					 "JIG_UARTOFF_VB ATTACHED*****\n");
			} else {
				dev_classifi = SM5502_CABLE_TYPE2_JIG_UART_OFF;
				dev_info(&client->dev,
		...
		if (val2 & DEV_JIG_UART_ON) {
			if (vbusin & VBUSIN_VALID) {
				dev_classifi = SM5502_CABLE_TYPE2_JIG_UART_ON_VB;
				dev_info(&client->dev,
					 "JIG_UARTON_VB ATTACHED*****\n");

			} else {
				dev_classifi = SM5502_CABLE_TYPE2_JIG_UART_ON;
				dev_info(&client->dev,
					 "JIG_UARTON ATTACHED*****\n");
			}
			additional_vbus_int_enable(usbsw);
		...
		if (val2 & DEV_JIG_USB_OFF && vbusin & VBUSIN_VALID) {
			dev_classifi = SM5502_CABLE_TYPE2_JIG_USB_OFF;
			dev_info(&client->dev, "JIG_USB_OFF ATTACHED*****\n");
		}
		if (val2 & DEV_JIG_USB_ON && vbusin & VBUSIN_VALID) {
			dev_classifi = SM5502_CABLE_TYPE2_JIG_USB_ON;
		...
		if (val2 & DEV_AV) {
			...
			if (val3 & DEV_AV_VBUS && vbusin & VBUSIN_VALID) {
				dev_classifi = SM5502_CABLE_TYPE3_DESKDOCK_VB;
				dev_info(&client->dev,
					 "DESKDOCK+VBUS ATTACHED*****\n");
			} else {
				dev_classifi = SM5502_CABLE_TYPE2_DESKDOCK;
				dev_info(&client->dev,
					 "DESKDOCK ATTACHED*****\n");
			}
			additional_vbus_int_enable(usbsw);
...
#ifdef CONFIG_EXTCON_SAMSUNG
		extcon_samsung_call_chain(SM5502_CABLE_TYPE_NONE);
#endif
		reset_count = 0;
	}
...
static int sm5502_resume(struct i2c_client *client)
{
	...
	dev_info(&client->dev, "sm5502_resume\n");
