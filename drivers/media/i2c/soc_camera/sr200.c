...
static int sr200_write_raw_array(struct v4l2_subdev *sd,
				  const struct sr200_regval *msg, char *name)
{
	...
	while (1) {
		if(msg->addr == SR200_TERM && msg->val == 0)
			break;
		else if(msg->addr == SR200_TERM && msg->val != 0)		
		{
			msleep(msg->val*10);
			msg++;
			continue;
...
static int sr200_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable)
		ret = sr200_write_raw_array(sd, regs_start_stream, "regs_start_stream");
	else
		ret = sr200_write_raw_array(sd, regs_stop_stream, "regs_stop_stream");
	return ret;
}
			 ...
			 struct v4l2_mbus_framefmt *mf)
{
	...
	case NORMAL_TO_VIDEO:
		...
		break;
	case VIDEO_TO_CALL:
		regs_resolution_name = "regs_res_vga_vt";
		break;
	default:
		if (mf->width == 640)
	 ...
	 */
	ret = sr200_write_raw_array(sd, regs_resolution, regs_resolution_name);
...
static int sr200_init(struct v4l2_subdev *sd, u32 plat)
{
	...
	ret = sr200_write_raw_array(sd, regs_start_setting, "regs_start_setting");
	return ret;
}
