...
#ifdef SEC_HEADSET_DET_IC
#include <linux/of_gpio.h>
#endif
#ifdef SEC_USE_SAMSUNG_JACK_API
...
struct pm886_hs_info {
	...
	int micbias_voltage; //KSND Patch for LDO 2.8V
...
static void mic_set_power(int on, struct pm886_hs_info *info)
{
	...
	if (on)
	{
		printk("set micbias_voltage=%d\n",info->micbias_voltage);
		regulator_set_voltage(info->mic_bias, info->micbias_voltage, info->micbias_voltage);
...
#ifdef SEC_HEADSET_DET_IC
    //KSND Get the Detect GPIO State
    if (info->det_gpio < 0) {
        dev_err(info->dev, "Fail to read detection GPIO value");
        return;
    }
    else {
        ret = gpio_get_value(info->det_gpio);
        value = (ret)? 0 : 1;
        pr_info("[headset] detection gpio=%d(0x%08x)\n", value, value);
    }
#else
...
static int pm886_headset_probe(struct platform_device *pdev)
{
    ...
    hs_info = devm_kzalloc(&pdev->dev, sizeof(struct pm886_hs_info), GFP_KERNEL);
	if (!hs_info)
		return -ENOMEM;

#ifdef SEC_HEADSET_DET_IC
    /* Initialize Ear detection IC  detection GPIO configuration    */
    hs_info->det_gpio = of_get_named_gpio(pdev->dev.of_node, "marvell,detection-gpio",0);
    pr_info("[headset] eardetection IC detection_gpio = %d, 0x%08x\n", hs_info->det_gpio, hs_info->det_gpio);
    ret = gpio_request(hs_info->det_gpio, "hs-det-gpio");

    if(ret < 0 ) {
        pr_err("[headset] of_get_named_gpio fail for detection-gpio\n");
        goto gpio_err;
    } else {
        gpio_direction_input(hs_info->det_gpio);
    }

    irq_headset = gpio_to_irq(hs_info->det_gpio);
    if (irq_headset < 0) {
        dev_err(&pdev->dev, "No IRQ resource for headset!\n");
        return -EINVAL;
    }

    /* Initialize Ear detection IC  key GPIO configuration  */
    hs_info->key_gpio = of_get_named_gpio(pdev->dev.of_node, "marvell,hskey-gpio",0);
    pr_info("[headset] eardetection IC key_gpio = %d, 0x%08x\n", hs_info->key_gpio, hs_info->key_gpio);
    ret = gpio_request(hs_info->key_gpio, "hs-key-gpio");

    if(ret < 0 ) {
        pr_err("[headset] of_get_named_gpio fail for key-gpio\n");
        goto gpio_err;
    }else{
        gpio_direction_input(hs_info->key_gpio);
    }

    irq_hook = gpio_to_irq(hs_info->key_gpio);
    if (irq_hook < 0) {
        dev_err(&pdev->dev, "No IRQ resource for hook/mic!\n");
        return -EINVAL;
    }
	...
	if (IS_ENABLED(CONFIG_OF)) {
		...
		if (of_property_read_u32(pdev->dev.of_node,
			"marvell,micbias-voltage", &hs_info->micbias_voltage))
			printk("Do not get micbias-voltage\n");
	} else {
		...
		hs_info->gpio5v_detect_mode = 0;
		hs_info->micbias_voltage = 2800000;
		printk("use default micbias-voltage");
	}
...
#ifdef SEC_HEADSET_DET_IC //KSND
    ret = devm_request_threaded_irq(&pdev->dev, hs_info->irq_headset,
            NULL, pm886_headset_handler,
            IRQF_ONESHOT | IRQF_TRIGGER_RISING |
            IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND,
            "ear-ic-detection", hs_info);

    if (ret < 0) {
        dev_err(chip->dev, "[headset][headset] Failed to request ear-ic-detection IRQ: #%d: %d\n",
    ...
    ret = devm_request_threaded_irq(&pdev->dev, hs_info->irq_hook,
            ...
            "ear-ic-key", hs_info);
    if (ret < 0) {
        dev_err(chip->dev, "[headset] Failed to request ear-ic-key IRQ: #%d: %d\n",
...
#ifdef SEC_HEADSET_DET_IC
gpio_err:
    gpio_free(hs_info->det_gpio);
    gpio_free(hs_info->key_gpio);
#endif
