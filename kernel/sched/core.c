...
#ifdef CONFIG_SEC_DEBUG
	print_ftrace("[sched] prev_comm=%s prev_pid=%d prev_prio=%d prev_state=%ld ==> next_comm=%s next_pid=%d next_prio=%d caller=%x@",
		prev->comm, prev->pid, prev->prio,
		prev->state & (TASK_STATE_MAX-1),
		next->comm, next->pid, next->prio, CALLER_ADDR0);
#endif
